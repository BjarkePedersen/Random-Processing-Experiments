...
void showBoard() {
  ...
  fill(10, 70, 200);
  rect(offsetY-border1, offsetX-border1, width-offsetY*2+border1*2, height-offsetX*2+border1*2, 20, 20, 0, 0);
  for (int i=0; i<GW; i++)
    for (int j=0; j<GH; j++) {
      fill(25);
...
void showWinner(boolean[][] winningBricks) {
  ...
  textAlign(CENTER);
  text((winner == p1 ? "Red" : "Yellow") + " wins!", width/2, 40);
