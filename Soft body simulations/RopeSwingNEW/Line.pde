...
public class Line {
  ...
  void show(int i, String type) {
    line(v1.loc.x, v1.loc.y, v2.loc.x, v2.loc.y);
    //beginShape();
    //if (i == 0 ) curveVertex(verts[i].loc.x, verts[i].loc.y);
    //else         curveVertex(verts[i-1].loc.x, verts[i-1].loc.y);
    //curveVertex(verts[i].loc.x, verts[i].loc.y);
    //curveVertex(verts[i+1].loc.x, verts[i+1].loc.y);
    //curveVertex(verts[i+2].loc.x, verts[i+2].loc.y);
    //curveVertex(verts[i+3].loc.x, verts[i+3].loc.y);
    //endShape();
  }

  float getLength() {
    return sqrt(pow(v1.loc.x-v2.loc.x, 2) + pow(v1.loc.y-v2.loc.y, 2));
  }
}
