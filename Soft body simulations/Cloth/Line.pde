public class Line {
  ...
  float push = .99;  // Resistance to expanding
  float pull = .99;  // Resistance to pulling
  float damp = .7;  // Compression / expansion dampening
  ...
  void calcForces() {
    ...
    // Compensate forces for fixed particles 
    if (v1.fixed) { 
      v1dir.mult(0);
      v2dir.mult(4);
    }
