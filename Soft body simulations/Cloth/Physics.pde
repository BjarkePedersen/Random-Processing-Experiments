  ...
  for (int i=0; i<verts.length; i++) {
    for (int j=0; j<verts[i].length; j++) {
      ...
      for (int x=0; x<verts.length; x++) {
        ...
        verts[x][0].loc = new PVector(50+x*ropeElmLen, 100);
        verts[x][0].fixed = true;
      }
      if (mousePressed) {
        verts[verts.length/2][verts[verts.length/2].length/2].vel = new PVector(0, 0);
        verts[verts.length/2][verts[verts.length/2].length/2].fixed = true;
        verts[verts.length/2][verts[verts.length/2].length/2].loc = new PVector(mouseX, mouseY);
        verts[verts.length/2][verts[verts.length/2].length/2].mass = 9999;
      } else {
        verts[verts.length/2][verts[verts.length/2].length/2].mass = 1;
        verts[verts.length/2][verts[verts.length/2].length/2].fixed = false;
      }
